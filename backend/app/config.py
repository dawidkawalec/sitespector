...
class Settings(BaseSettings):
    ...
    # Legacy price IDs (backward compat — maps to Solo/Enterprise)
    STRIPE_PRICE_ID_PRO: str = Field(default="", description="[DEPRECATED] Stripe price ID for Pro plan → Solo")
    STRIPE_PRICE_ID_ENTERPRISE: str = Field(default="", description="[DEPRECATED] Stripe price ID for Enterprise plan")
    # New 5-plan model: Solo / Agency / Enterprise × monthly / annual
    STRIPE_PRICE_ID_SOLO_MONTHLY: str = Field(default="", description="Stripe price ID: Solo $9.99/msc")
    STRIPE_PRICE_ID_SOLO_ANNUAL: str = Field(default="", description="Stripe price ID: Solo $7.99/msc annual")
    STRIPE_PRICE_ID_AGENCY_MONTHLY: str = Field(default="", description="Stripe price ID: Agency $29.99/msc")
    STRIPE_PRICE_ID_AGENCY_ANNUAL: str = Field(default="", description="Stripe price ID: Agency $23.99/msc annual")
    STRIPE_PRICE_ID_ENTERPRISE_MONTHLY: str = Field(default="", description="Stripe price ID: Enterprise $99/msc")
    STRIPE_PRICE_ID_ENTERPRISE_ANNUAL: str = Field(default="", description="Stripe price ID: Enterprise $79/msc annual")
    # Credit packages (one-time payments)
    STRIPE_PRICE_ID_CREDITS_STARTER: str = Field(default="", description="Stripe price: 50 kr / $4.99")
    STRIPE_PRICE_ID_CREDITS_STANDARD: str = Field(default="", description="Stripe price: 150 kr / $12.99")
    STRIPE_PRICE_ID_CREDITS_PRO: str = Field(default="", description="Stripe price: 500 kr / $34.99")
    STRIPE_PRICE_ID_CREDITS_AGENCY: str = Field(default="", description="Stripe price: 1500 kr / $89.99")
