...
class Persona(Base):
    """Persona definition — extensible system of audit perspectives."""
    ...
    icon = Column(String(60), nullable=True)  # Lucide icon name
    sort_order = Column(Integer, default=0, nullable=False)

    # AI configuration
    prompt_modifier = Column(Text, nullable=True)  # Injected into all AI prompts
    dashboard_config = Column(JSONB, nullable=True)  # {kpi_cards, focus_modules}
    context_questions = Column(JSONB, nullable=True)  # Persona-specific questions for creation
    ...
    def __repr__(self) -> str:
        return f"<Persona(slug={self.slug}, name={self.name})>"
...
class Audit(Base):
    ...
    # Persona (Phase B: multi-persona system)
    ...
    action_cards = relationship("ActionCard", back_populates="audit", cascade="all, delete-orphan")
...
class ActionCard(Base):
    """Interactive action card generated by AI for persona dashboard."""
    ...
    title = Column(String(500), nullable=False)
    description = Column(Text, nullable=False)
    category = Column(String(50), nullable=True)  # performance, seo, content, ux, security
    priority = Column(String(20), nullable=True)  # critical, high, medium, low
    kpi_impact = Column(JSONB, nullable=True)  # {"metric": "lcp", "improvement": "-500ms"}
    action_data = Column(JSONB, nullable=True)  # Code snippets, URLs, structured data

    # Lifecycle
    status = Column(String(20), default="suggested", nullable=False, index=True)
    source = Column(String(30), nullable=False)  # auto_generation, chat_created, user_created
    ...
    # Relationships
    audit = relationship("Audit", back_populates="action_cards")

    def __repr__(self) -> str:
        return f"<ActionCard(id={self.id}, title={self.title[:40]}, status={self.status})>"
